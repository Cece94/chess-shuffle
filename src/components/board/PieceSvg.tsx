import type { ReactNode } from 'react'
import type { Color, PieceSymbol } from '@/lib/chess/types'

type Props = {
  type: PieceSymbol
  color: Color
  className?: string
}

/** Classic Staunton-style SVG silhouettes for the 2D board. */
export function PieceSvg({ type, color, className }: Props) {
  const isWhite = color === 'w'
  const fill = isWhite ? '#f3efe6' : '#1a1510'
  const stroke = isWhite ? '#2a241c' : '#c4a35a'

  return (
    <svg
      viewBox="0 0 45 45"
      className={className}
      aria-hidden
      style={{ filter: isWhite ? 'drop-shadow(0 1px 1px rgba(0,0,0,0.35))' : undefined }}
    >
      <g fill={fill} stroke={stroke} strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
        {PATHS[type](fill, stroke)}
      </g>
    </svg>
  )
}

const PATHS: Record<PieceSymbol, (fill: string, stroke: string) => ReactNode> = {
  p: () => (
    <path d="M22.5 9c-2.21 0-4 1.79-4 4 0 .89.29 1.71.78 2.38C17.33 16.5 16 18.59 16 21c0 2.03.94 3.84 2.41 5.03A5 5 0 0 0 16 31v2h13v-2a5 5 0 0 0-2.41-4.97C28.06 24.84 29 23.03 29 21c0-2.41-1.33-4.5-3.28-5.62.49-.67.78-1.49.78-2.38 0-2.21-1.79-4-4-4z" />
  ),
  r: (fill) => {
    const isWhite = fill === '#f3efe6'
    if (isWhite) {
      return (
        <g fillRule="evenodd">
          <path
            strokeLinecap="butt"
            d="M9 39h27v-3H9zm3-3v-4h21v4zm-1-22V9h4v2h5V9h5v2h5V9h4v5"
          />
          <path d="m34 14-3 3H14l-3-3" />
          <path strokeLinecap="butt" strokeLinejoin="miter" d="M31 17v12.5H14V17" />
          <path d="m31 29.5 1.5 2.5h-20l1.5-2.5" />
          <path fill="none" strokeLinejoin="miter" d="M11 14h23" />
        </g>
      )
    }
    return (
      <g fillRule="evenodd">
        <path
          strokeLinecap="butt"
          d="M9 39h27v-3H9zm3.5-7 1.5-2.5h17l1.5 2.5zm-.5 4v-4h21v4z"
        />
        <path strokeLinecap="butt" strokeLinejoin="miter" d="M14 29.5v-13h17v13z" />
        <path strokeLinecap="butt" d="M14 16.5 11 14h23l-3 2.5zM11 14V9h4v2h5V9h5v2h5V9h4v5z" />
        <path
          fill="none"
          stroke="#c4a35a"
          strokeLinejoin="miter"
          strokeWidth={1}
          d="M12 35.5h21m-20-4h19m-18-2h17m-17-13h17M11 14h23"
        />
      </g>
    )
  },
  // Classic CBurnett horse head — much more readable at board size
  n: (fill) => {
    const isWhite = fill === '#f3efe6'
    const eye = isWhite ? '#2a241c' : '#ececec'
    return (
      <g fillRule="evenodd">
        <path
          fill={fill}
          d="M22 10c10.5 1 16.5 8 16 29H15c0-9 10-6.5 8-21"
        />
        <path
          fill={fill}
          d="M24 18c.38 2.91-5.55 7.37-8 9-3 2-2.82 4.34-5 4-1.042-.94 1.41-3.04 0-3-1 0 .19 1.23-1 2-1 0-4.003 1-4-4 0-2 6-12 6-12s1.89-1.9 2-3.5c-.73-.994-.5-2-.5-3 1-1 3 2.5 3 2.5h2s.78-1.992 2.5-3c1 0 1 3 1 3"
        />
        <path
          fill={eye}
          stroke={eye}
          d="M9.5 25.5a.5.5 0 1 1-1 0 .5.5 0 1 1 1 0m5.433-9.75a.5 1.5 30 1 1-.866-.5.5 1.5 30 1 1 .866.5"
        />
        {!isWhite && (
          <path
            fill="#ececec"
            stroke="none"
            d="m24.55 10.4-.45 1.45.5.15c3.15 1 5.65 2.49 7.9 6.75S35.75 29.06 35.25 39l-.05.5h2.25l.05-.5c.5-10.06-.88-16.85-3.25-21.34s-5.79-6.64-9.19-7.16z"
          />
        )}
      </g>
    )
  },
  b: () => (
    <>
      <g fillRule="evenodd">
        <path d="M9 36c3.39-.97 10.11.43 13.5-2 3.39 2.43 10.11 1.03 13.5 2 0 0 1.65.54 1.65 2.03H7.35C7.35 36.54 9 36 9 36z" />
        <path d="M15 32c2.5 2.5 12.5 2.5 15 0 .5-1.5 0-2 0-2 0-2.5-2.5-4-2.5-4 5.5-1.5 6-11.5-5-15.5-11 4-10.5 14-5 15.5 0 0-2.5 1.5-2.5 4 0 0-.5.5 0 2z" />
        <path d="M25 8a2.5 2.5 0 1 1-5 0 2.5 2.5 0 1 1 5 0z" />
      </g>
      <path d="M17.5 26h10M15 30h15M22.5 15.5l-3 8.5M22.5 15.5l3 8.5" fill="none" />
    </>
  ),
  q: (fill) => {
    const isWhite = fill === '#f3efe6'
    if (isWhite) {
      return (
        <g fillRule="evenodd">
          <path d="M8 12a2 2 0 1 1-4 0 2 2 0 1 1 4 0m16.5-4.5a2 2 0 1 1-4 0 2 2 0 1 1 4 0M41 12a2 2 0 1 1-4 0 2 2 0 1 1 4 0M16 8.5a2 2 0 1 1-4 0 2 2 0 1 1 4 0M33 9a2 2 0 1 1-4 0 2 2 0 1 1 4 0" />
          <path
            strokeLinecap="butt"
            d="M9 26c8.5-1.5 21-1.5 27 0l2-12-7 11V11l-5.5 13.5-3-15-3 15-5.5-14V25L7 14z"
          />
          <path
            strokeLinecap="butt"
            d="M9 26c0 2 1.5 2 2.5 4 1 1.5 1 1 .5 3.5-1.5 1-1.5 2.5-1.5 2.5-1.5 1.5.5 2.5.5 2.5 6.5 1 16.5 1 23 0 0 0 1.5-1 0-2.5 0 0 .5-1.5-1-2.5-.5-2.5-.5-2 .5-3.5 1-2 2.5-2 2.5-4-8.5-1.5-18.5-1.5-27 0z"
          />
          <path fill="none" d="M11.5 30c3.5-1 18.5-1 22 0M12 33.5c6-1 15-1 21 0" />
        </g>
      )
    }
    return (
      <g fillRule="evenodd">
        <g stroke="none">
          <circle cx="6" cy="12" r="2.75" />
          <circle cx="14" cy="9" r="2.75" />
          <circle cx="22.5" cy="8" r="2.75" />
          <circle cx="31" cy="9" r="2.75" />
          <circle cx="39" cy="12" r="2.75" />
        </g>
        <path
          strokeLinecap="butt"
          d="M9 26c8.5-1.5 21-1.5 27 0l2.5-12.5L31 25l-.3-14.1-5.2 13.6-3-14.5-3 14.5-5.2-13.6L14 25 6.5 13.5z"
        />
        <path
          strokeLinecap="butt"
          d="M9 26c0 2 1.5 2 2.5 4 1 1.5 1 1 .5 3.5-1.5 1-1.5 2.5-1.5 2.5-1.5 1.5.5 2.5.5 2.5 6.5 1 16.5 1 23 0 0 0 1.5-1 0-2.5 0 0 .5-1.5-1-2.5-.5-2.5-.5-2 .5-3.5 1-2 2.5-2 2.5-4-8.5-1.5-18.5-1.5-27 0z"
        />
        <path fill="none" strokeLinecap="butt" d="M11 38.5a35 35 1 0 0 23 0" />
        <path
          fill="none"
          stroke="#c4a35a"
          d="M11 29a35 35 1 0 1 23 0m-21.5 2.5h20m-21 3a35 35 1 0 0 22 0m-23 3a35 35 1 0 0 24 0"
        />
      </g>
    )
  },
  k: () => (
    <>
      <path d="M22.5 11.63V6M20 8h5" fill="none" strokeWidth={1.75} />
      <path
        d="M22.5 25s4.5-7.5 3-10.5c0 0-1-2.5-3-2.5s-3 2.5-3 2.5c-1.5 3 3 10.5 3 10.5"
        fillRule="evenodd"
      />
      <path d="M12.5 37c5.5 3.5 14.5 3.5 20 0v-7s9-4.5 6-10.5c-4-6.5-13.5-3.5-16 4V27v-3.5c-3.5-7.5-13-10.5-16-4-3 6 5 10 5 10V37z" />
      <path d="M12.5 30c5.5-3 14.5-3 20 0M12.5 33.5c5.5-3 14.5-3 20 0M12.5 37c5.5-3 14.5-3 20 0" fill="none" />
    </>
  ),
}
