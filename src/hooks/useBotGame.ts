...
function freshPosition(level: BotLevel, playerColor: Color): BotGameSnapshot {
...
export function useBotGame() {
  ...
  const rematch = useCallback(() => {
    ...
    clearBotTimer()
    genRef.current += 1
    // Swap colors like online rematch
    const nextColor: Color = state.playerColor === 'w' ? 'b' : 'w'
    setState(freshPosition(state.level, nextColor))
  }, [clearBotTimer, state.level, state.playerColor])
  ...
  useEffect(() => {
    ...
    botTimer.current = setTimeout(() => {
      ...
      if (!result.ok || genRef.current !== gen) {
        setState((s) => ({ ...s, botThinking: false }))
        return
      }
